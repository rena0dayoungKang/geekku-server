...
public class FcmMessageService {
	...
	public Boolean sendHouseAnswer(HouseAnswerDto houseAnswerDto) throws Exception {
		...
		Message message = Message.builder()
				...
				.putData("message", alarm.getMessage())
	...
	public Boolean sendInteriorAllAnswer(InteriorAnswerDto interiorAnswerDto) throws Exception {
		...
		AlarmUser alarm = AlarmUser.builder()
		    ...
		    .message(interiorAnswerDto.getTitle())
		    .status(false)
		    .type("interior")
		    .detailPath(interiorAnswerDto.getAnswerAllNum())
		    .build();
	...
	public Boolean sendOnstopAnswer(OnestopAnswerDto onestopAnswerDto) throws Exception {
		...
		AlarmUser alarm = AlarmUser.builder()
		    ...
		    .message(onestopAnswerDto.getTitle())
		    .status(false)
		    .type("interior")
		    .detailPath(onestopAnswerDto.getAnswerOnestopNum())
		    .build();
	...
	public Boolean sendInteriorRequest(InteriorRequestDto interiorRequestDto) throws Exception {
		...
		Company company = companyRepository.findById(interiorRequestDto.getCompanyId()).orElseThrow(()->new Exception("Company 오류"));
		...
		//3. AlarmTable에 저장
		AlarmUser alarm = AlarmUser.builder()
		    .user(interiorRequestDto.getUserId()) // UUID 타입 전달
		    .company(company)
		    .message(interiorRequestDto.getName()+"님으로부터 인테리어 문의")
		    .status(false)
		    .type("request")
		    .detailPath(interiorRequestDto.getRequestNum())
		    .build();
	...
	public List<MessageDto> getUserAlarmList(UUID userId) {
		...
		List<MessageDto> A = alarmList.stream().map(alarm->MessageDto.builder()
									.num(alarm.getUserAlarmNum())
									.receiver(alarm.getUser().getUserId())
	...
	public List<MessageDto> getCompanyAlarmList(UUID comapnyId) {
		...
		return alarmList.stream().map(alarm->MessageDto.builder()
									.num(alarm.getUserAlarmNum())
									.receiver(alarm.getUser().getUserId())
									.message(alarm.getMessage())
									.createAt(alarm.getCreatedAt())
	...
	public void registCompanyFcmToken(String companyId, String fcmToken) throws Exception {
		Company company = companyRepository.findById(UUID.fromString(companyId)).orElseThrow(()->new Exception("Company Err"));
